...
void QupZillaSchemeHandler::requestStarted(QWebEngineUrlRequestJob *job)
{
    ...
    knownPages << "about" << "reportbug" << "start" << "speeddial" << "config" << "restore" << "reader";
...
void QupZillaSchemeReply::loadPage()
{
    ...
    else if (m_pageName == QLatin1String("reader")) {
        stream << readerPage();
    }
...
QString QupZillaSchemeReply::startPage()
{
    ...
    // Read PNG bytes directly and base64 encode (avoids image format plugin dependency)
    ...
    sPage.replace(QLatin1String("%ABOUT-IMG%"), imgDataUrl);
...
QString QupZillaSchemeReply::aboutPage()
{
    ...
    if (aPage.isEmpty()) {
        ...
        // Read PNG bytes directly and base64 encode (avoids image format plugin dependency)
        QString aboutImgDataUrl = QSL("data:image/png;base64,") +
            QString::fromLatin1(QzTools::readAllFileByteContents(QSL(":icons/other/about.png")).toBase64());
        aPage.replace(QLatin1String("%ABOUT-IMG%"), aboutImgDataUrl);
...
QString QupZillaSchemeReply::configPage()
{
    ...
    if (cPage.isEmpty()) {
        ...
        // Read PNG bytes directly and base64 encode (avoids image format plugin dependency)
        QString configImgDataUrl = QSL("data:image/png;base64,") +
            QString::fromLatin1(QzTools::readAllFileByteContents(QSL(":icons/other/about.png")).toBase64());
        cPage.replace(QLatin1String("%ABOUT-IMG%"), configImgDataUrl);
...
QString QupZillaSchemeReply::readerPage()
{
    QString page = QzTools::readAllFileContents(QSL(":readingmode/reader.html"));

    // Parse the original URL from query string
    QUrlQuery query(m_job->requestUrl());
    QString encodedUrl = query.queryItemValue(QSL("url"));
    QUrl originalUrl;
    if (!encodedUrl.isEmpty()) {
        originalUrl = QUrl::fromEncoded(QByteArray::fromBase64(encodedUrl.toUtf8()));
    }

    // Get cached article content
    ReadingModeArticle article = ReadingModeManager::instance()->getCachedArticle(originalUrl);

    // Load theme CSS for reading mode
    Settings settings;
    settings.beginGroup(QSL("Themes"));
    QString activeTheme = settings.value(QSL("activeTheme"), QSL("linux")).toString();
    settings.endGroup();

    QString themeCss;
    const QStringList themePaths = DataPaths::allPaths(DataPaths::Themes);
    for (const QString &path : themePaths) {
        const QString cssPath = QString(QSL("%1/%2/readingmode.css")).arg(path, activeTheme);
        if (QFile::exists(cssPath)) {
            themeCss = QzTools::readAllFileContents(cssPath);
            break;
        }
    }

    page.replace(QLatin1String("%THEME-CSS%"), themeCss);
    page.replace(QLatin1String("%TITLE%"), tr("Reading Mode"));
    page.replace(QLatin1String("%BACK-LABEL%"), tr("Back to page"));
    page.replace(QLatin1String("%LOADING%"), tr("Loading"));
    page.replace(QLatin1String("%ERROR-MSG%"), tr("Could not load article content."));
    page.replace(QLatin1String("%SOURCE-LABEL%"), tr("Source"));
    page.replace(QLatin1String("%ORIGINAL-URL%"), originalUrl.toString().toHtmlEscaped());
    page.replace(QLatin1String("%ORIGINAL-URL-ENCODED%"), QString::fromUtf8(QUrl::toPercentEncoding(originalUrl.toString())));

    // If we have cached content, inject it via JavaScript
    if (article.isValid()) {
        ...
        QString byline = article.byline;
        byline.replace(QLatin1Char('\\'), QLatin1String("\\\\"));
        byline.replace(QLatin1Char('\''), QLatin1String("\\'"));
        byline.replace(QLatin1Char('\n'), QLatin1String("\\n"));

        QString content = article.content;
        content.replace(QLatin1Char('\\'), QLatin1String("\\\\"));
        content.replace(QLatin1Char('\''), QLatin1String("\\'"));
        content.replace(QLatin1Char('\n'), QLatin1String("\\n"));
        content.replace(QLatin1String("</script>"), QLatin1String("<\\/script>"));

        QString injectScript = QSL(
            "<script>"
            "window.addEventListener('DOMContentLoaded', function() {"
            "  window.displayArticle('%1', '%2', '%3');"
            "});"
            "</script>"
        ).arg(title, content, byline);

        page.replace(QLatin1String("</body>"), injectScript + QLatin1String("</body>"));
    } else {
        // Show error if no cached content
        QString errorScript = QSL(
            "<script>"
            "window.addEventListener('DOMContentLoaded', function() {"
            "  window.showError();"
            "});"
        ...
        page.replace(QLatin1String("</body>"), errorScript + QLatin1String("</body>"));
    }

    return QzTools::applyDirectionToPage(page);
}
