    ...
    , m_pluginsList(0)
{
    ...
    // No session restore in portable build - hide related UI completely
    ui->afterLaunch->hide();
    ui->label_3->hide();
    ui->dontLoadTabsUntilSelected->hide();

    // No profile management in portable build - hide related UI completely
    ui->label_4->hide();
    ui->label_45->hide();
    ui->activeProfile->hide();
    ui->label_5->hide();
    ui->startProfile->hide();
    ui->createProfile->hide();
    ui->deleteProfile->hide();
    ui->cannotDeleteActiveProfileLabel->hide();

    // No language selection in portable build - hide related UI completely
    ui->label_11->hide();
    ui->label_16->hide();
    ui->label_17->hide();
    ui->languages->hide();
...
#else
    ui->wheelScroll->hide();
    ui->label_14->hide();  // "Mouse wheel scrolls"
    ui->label_26->hide();  // "lines on page"
#endif
