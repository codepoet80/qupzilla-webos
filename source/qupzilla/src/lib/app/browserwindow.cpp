...
static bool bwLoggingEnabled() {
    struct stat st;
    return (stat("/media/internal/qupzilla/enable_logging", &st) == 0);
}

static void bwLog(const char* msg) {
    if (!bwLoggingEnabled()) return;
...
void BrowserWindow::setupUi()
{
    ...
    ReadingModeIcon *readingModeIcon = new ReadingModeIcon(this);
    m_statusBar->addButton(readingModeIcon);
    m_navigationToolbar->addToolButton(readingModeIcon);
...
{
    // Defensive check - UI may not be fully initialized during deferred calls
...
void BrowserWindow::updateLoadingActions()
{
    ...
    // Defensive checks - UI elements may not be initialized during deferred calls
    if (!m_ipLabel || !m_progressBar || !m_navigationToolbar) {
        return;
    }
    ...
    QAction* stopAction = action(QSL("View/Stop"));
    QAction* reloadAction = action(QSL("View/Reload"));
    if (stopAction) {
        stopAction->setEnabled(isLoading);
    }
    if (reloadAction) {
        reloadAction->setEnabled(!isLoading);
    }
