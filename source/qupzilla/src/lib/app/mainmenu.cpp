...
void MainMenu::initSuperMenu(QMenu* superMenu) const
{
    ...
    superMenu->addAction(m_actions[QSL("Tools/JavaScript")]);
...
#define ADD_CHECKABLE_ACTION(name, menu, icon, trName, slot, shortcut) \
    ...
    // Load Images toggle (for SuperMenu - created here so action is available)
    ...
    // JavaScript toggle (for SuperMenu)
    action = new QAction(tr("JavaScript"), this);
    action->setCheckable(true);
    action->setChecked(mApp->webSettings()->testAttribute(QWebEngineSettings::JavascriptEnabled));
    connect(action, &QAction::toggled, this, &MainMenu::toggleJavaScript);
    m_actions[QSL("Tools/JavaScript")] = action;
...
void MainMenu::toggleLoadImages(bool enable)
{
    mApp->webSettings()->setAttribute(QWebEngineSettings::AutoLoadImages, enable);
    Settings settings;
    settings.beginGroup("Web-Browser-Settings");
    settings.setValue("autoLoadImages", enable);
    settings.endGroup();
}

void MainMenu::toggleJavaScript(bool enable)
{
    mApp->webSettings()->setAttribute(QWebEngineSettings::JavascriptEnabled, enable);
    Settings settings;
    settings.beginGroup("Web-Browser-Settings");
    settings.setValue("allowJavaScript", enable);
    settings.endGroup();
}

void MainMenu::aboutToShowUserAgentMenu()
{
    QMenu* menu = qobject_cast<QMenu*>(sender());
    if (!menu) {
        return;
    ...
    QString currentUA = mApp->webProfile()->httpUserAgent();

    struct UAEntry { QString name; QString ua; };
    QVector<UAEntry> agents = {
        {tr("Default (QupZilla)"), QString()},
        {tr("Chrome (Windows)"), QSL("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36")},
        {tr("Firefox (Windows)"), QSL("Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:121.0) Gecko/20100101 Firefox/121.0")},
        {tr("Safari (macOS)"), QSL("Mozilla/5.0 (Macintosh; Intel Mac OS X 14_2) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.2 Safari/605.1.15")},
        {tr("Edge (Windows)"), QSL("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Edg/120.0.0.0")},
        {tr("Chrome (Android)"), QSL("Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36")},
        {tr("Safari (iPad)"), QSL("Mozilla/5.0 (iPad; CPU OS 17_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.2 Mobile/15E148 Safari/604.1")}
    };

    // Get the default UA for comparison (empty string means default)
    Settings settings;
    settings.beginGroup("Web-Browser-Settings");
    QString savedUA = settings.value("UserAgent", QString()).toString();
    settings.endGroup();
    ...
    for (const auto& agent : agents) {
        ...
        action->setCheckable(true);
        action->setData(agent.ua);
        group->addAction(action);
        // Check if this is the current UA
        if (agent.ua.isEmpty() && savedUA.isEmpty()) {
            action->setChecked(true);
        } else if (agent.ua == savedUA) {
            action->setChecked(true);
        }
    }
    connect(group, &QActionGroup::triggered, this, &MainMenu::setUserAgent);
}

void MainMenu::setUserAgent(QAction* action)
{
    QString ua = action->data().toString();

    if (ua.isEmpty()) {
        // Reset to default - clear the custom UA
        mApp->webProfile()->setHttpUserAgent(QString());
    } else {
        mApp->webProfile()->setHttpUserAgent(ua);
    }

    // Save to settings
    Settings settings;
    settings.beginGroup("Web-Browser-Settings");
    settings.setValue("UserAgent", ua);
    settings.endGroup();
}
