    ...
    : QWidget()
{
    ...
    // Pre-initialize empty web image in main thread to avoid QPixmap threading issues
    // Create pixmap with explicit transparent background for dark themes
    QPixmap pm(16, 16);
    pm.fill(Qt::transparent);
    QPainter painter(&pm);
    QIcon(QSL(":icons/other/webpage.svg")).paint(&painter, 0, 0, 16, 16);
    m_emptyWebImage = pm.toImage();
}
...
QImage IconProvider::emptyWebImage()
{
    if (instance()->m_emptyWebImage.isNull()) {
        ...
        QPainter painter(&pm);
        QIcon(QSL(":icons/other/webpage.svg")).paint(&painter, 0, 0, 16, 16);
        instance()->m_emptyWebImage = pm.toImage();
    }
